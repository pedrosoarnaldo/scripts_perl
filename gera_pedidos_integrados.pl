...
while() {
  ...
  $query = "INSERT INTO Pedidos_Integrados (data_integracao, quantidade_integrada)  VALUES(?, ?)";
...
#  print $log "server1.intranet,$time,$randon_number\n";
  ...
  $system_variable = "zabbix_sender -z stark -s server1.intranet -k PPM -o $randon_number";

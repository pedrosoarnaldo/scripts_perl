...
while() {
  ...
  $query = $dbh->prepare('select quantidade_minutos + 1 from NF_Emitida where data_integracao = ( select max(data_integracao) from Pedidos_Aprovados )');
  $query->execute();
  ...
  $query = "INSERT INTO Site_Transportadora (data_integracao, quantidade_minutos)  VALUES(?, ?)";
...
#  print $log "server1.intranet,$time,$randon_number\n";
  ...
  $system_variable = "zabbix_sender -z stark -s server6.intranet -k TR -o $randon_number";
